...
class Model:
    ...
    def advance_to(self, i): # i becomes self._current_event - 1
        self._reset()
    ...
    def remove(self, i):
        ...
        self._current_event -= 1
        self.advance_to(self._current_event - 1)
        pub.sendMessage("HISTORY_UPDATE", data=self.history_info())
...
class AddPokeEvent(Event):
    ...
    def run(self, model):
        ...
        model.d_moveset = self.poke.moveset()
        model.add_to_party(self.poke)

    def info(self, model):
        my_poke = model.cur_poke()
        if my_poke:
            return battle_output(model, my_poke, self.poke)
...
class KillEvent(Event):
    ...
    def info(self, model):
        return battle_output(model, self.pre_mine, self.theirs)
...
def battle_output(model, mine, theirs):
    """Generate battle output. Good for use in event info view.

    model: the model
    mine: my PartyPoke
    theirs: their PartyPoke    
    """
    ans = ''
    if dat.is_move(model.a_move_name):
        move_name = model.a_move_name
        dmg = mine.dmg(theirs, move_name,
        ...
        ans += 'Attack with ' + move_name + ' ' + mine.crit_chance(move_name) + '\n' + dmg + '\n'
    if dat.is_move(model.d_move_name):
        move_name = model.d_move_name
        dmg = theirs.dmg(mine, move_name,
        ...
        ans += 'Defend ' + move_name + ' ' + theirs.crit_chance(move_name) + '\n' + dmg + '\n'
