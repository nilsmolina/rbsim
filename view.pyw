...
#AUTOLOAD_FILE = os.path.join(os.getcwd(), 'autosave.sim') # If blank, no auto-loading on open.
#AUTOSAVE_FILE = os.path.join(os.getcwd(), 'autosave.sim') # If blank, no autosaving on close.
AUTOLOAD_FILE = 'autosave.sim'
AUTOSAVE_FILE = 'autosave.sim'
...
class TopFrame(wx.Frame):
    def __init__(self, model, *args, **kargs):
        ...
        if AUTOLOAD_FILE:
            self.model.t_open(AUTOLOAD_FILE)
    ...
    def on_close(self, e):
        if AUTOSAVE_FILE:
            # TODO move this into somewhere more frequently called...
            self.model.t_save(AUTOSAVE_FILE)
        self.destroy()
